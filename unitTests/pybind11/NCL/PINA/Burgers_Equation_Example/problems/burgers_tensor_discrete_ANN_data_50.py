    ...
    def burger_equation_derivative(self,input_, output_):
        ...
        for i in range(0,Tmax-1,5):
          ...
          for j in range(output_.size(1)): 
            ...
            a.updatephi() 
            Spatial_res = a.getResidual().reshape(-1,1)
            #print("Spatial_res ===", dt*Spatial_res)
            Spatial_res_tensor = (torch.from_numpy(Spatial_res)).float()
            
            """
            Anew =  a.getResidual_Matrix(U).toarray()
            bnew = a.getResidual_vector(U).reshape(-1,1)
            spatial_residual = np.matmul(Anew,U) + bnew

            print("spatial_residual is ===", spatial_residual)
            print("Spatial_res is ===", dt*Spatial_res)
            """
