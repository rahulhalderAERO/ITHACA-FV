    ...
    def burger_equation_derivative(self,input_, output_):
        ...
        for i in range(0,Tmax-1,10):
          ...
          for j in range(output_.size(1)): 
            ...
            dR_dUm1_Mat[:,j] = (R_Up[:,0]-R_Um[:,0])/(0.0002)
            Um[j,0] = Um[j,0] + 0.0001
          a.setU(U)
          a.setPrevU()
          a.updatephi() 
          dR_dU_Mat = a.get_system_matrix(U).toarray()
          dR_dU_Mat_Tensor = torch.from_numpy(dR_dU_Mat).float()          
